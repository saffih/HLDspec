...
def build_next_feature_agents_md(target: Path | str, report: dict[str, Any] | None = None) -> str:
    ...
    lines = [
        ...
        "",
        "1. Run the **target-local, read-only, self-contained** run-card wrapper from",
        "   inside this repo. It runs the vendored runtime under `.hldspec/runtime/`,",
        "   reads this repo's evidence, and prints the **SpecKit run card** -- the only",
        "   source of truth for *where we are*. No `HLDSPEC_HOME` and no HLDspec",
        "   checkout are needed; it never writes:",
        "",
        f"   ```",
        f"   {command}",
        f"   ```",
        ...
        f"   --apply` (it vendors the read-only runtime + wrapper). Only if you are",
        f"   working inside an HLDspec checkout, you may instead run:",
        "",
        f"   ```",
        f"   {fallback_command}",
        f"   ```",
        ...
        "   - **Agent / model guidance:** the driver's `agent_model_guidance` --",
        "     recommended actor, concrete model, thinking effort, whether a simple",
        "     low-cost model is allowed, and whether a human decision is required.",
        "     Show this so the human picks the right model before running the step.",
