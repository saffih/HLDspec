...
# Identity of the helper/toolchain this vendored runtime is an implementation of.
# Mirrors hldspec.helper_registry.build_speckit_helper(); only one operational
# helper exists today, so this is a literal, not a lookup into mutable state.
HELPER_ID = "speckit"
TOOLCHAIN = "SpecKit"
...
def _runtime_footer() -> str:
    ...
    if helper_id:
        lines.append(f"_helper {helper_id}_")
    if toolchain:
        lines.append(f"_toolchain {toolchain}_")
    return "\\n".join(lines) + "\\n"
