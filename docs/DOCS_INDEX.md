...
| [`TOOLCHAIN_DRIVER_EVIDENCE_CONTRACT.md`](TOOLCHAIN_DRIVER_EVIDENCE_CONTRACT.md) | PROPOSED: driver/toolchain/evidence-provenance boundary — `DRIVER_OBSERVED` vs `MANUAL_ATTESTED`/`DRIVER_VERIFIED_STATE` vs development receipts, why `next_feature_execution_evidence.json`'s existing entries are not driver-observed today, and why development receipts must never count as readiness evidence. Docs-only, no implementation. |
| [`SPECKIT_INVOCATION_AUDIT_LOG_CONTRACT.md`](SPECKIT_INVOCATION_AUDIT_LOG_CONTRACT.md) | RATIFIED design contract: durable, append-only STARTED/FINISHED lifecycle log for external SpecKit invocations at the pointer-aware `.hldspec/audit/speckit_invocations.jsonl`. Strictly separate from readiness evidence, provenance (`DRIVER_OBSERVED`/`MANUAL_ATTESTED`), proof records, and development receipts. Runtime implementation proceeds through five separately gated slices (A–E); Slices A–B (path/schema + durable append writer, both `hldspec/speckit_invocation_audit.py`) done, C–E not started; no production invocation path calls the writer and no audit record is produced automatically. |
