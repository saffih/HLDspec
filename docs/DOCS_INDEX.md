...
| [`SPECKIT_INVOCATION_AUDIT_LOG_CONTRACT.md`](SPECKIT_INVOCATION_AUDIT_LOG_CONTRACT.md) | RATIFIED design contract: durable, append-only STARTED/FINISHED lifecycle log for external SpecKit invocations at the pointer-aware `.hldspec/audit/speckit_invocations.jsonl`. Strictly separate from readiness evidence, provenance (`DRIVER_OBSERVED`/`MANUAL_ATTESTED`), proof records, and development receipts. Runtime implementation proceeds through five separately gated slices (A–E); Slice A (path/schema, `hldspec/speckit_invocation_audit.py`) done, B–E not started. |
