...
| [`SPECKIT_INVOCATION_AUDIT_LOG_CONTRACT.md`](SPECKIT_INVOCATION_AUDIT_LOG_CONTRACT.md) | RATIFIED design contract: durable, append-only STARTED/FINISHED lifecycle log for external SpecKit invocations at the pointer-aware `.hldspec/audit/speckit_invocations.jsonl`. Strictly separate from readiness evidence, provenance (`DRIVER_OBSERVED`/`MANUAL_ATTESTED`), proof records, and development receipts. Runtime implementation deferred through five separately gated slices (A–E); none started. |
