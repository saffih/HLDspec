...
1. ~~Human context evidence adapter~~: Addressed by `journey0_declared_evidence.py`. Converts explicit human declarations into typed product-surface EvidenceItems. Dry-run composition may accept this evidence explicitly; it is still not wired into the command surface.
