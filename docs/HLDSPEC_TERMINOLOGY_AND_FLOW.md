...
- **HLDspec Product Facade / Runtime Command Surface** — the canonical
  ...
  (alias `speckit-state`), `git-lifecycle` (git-read-only — writes evidence
  reports but never branches/commits/pushes/merges), `select-helper`, and
  ...
  **First-touch / brownfield rule:** on an unfamiliar or brownfield target the
  only safe read-only first command is `journey3-status`. `status`, `doctor`,
  `operator-state`, and `git-lifecycle` write discovery / git-lifecycle /
  branch-gate reports into the target's `.hldspec/` control state on first
  touch, so they are not read-only despite their inspection-style names.
...
| SpecKit Groundwork | `HLDspec doctor target: <path>` | current/read-only | `doctor`, `speckit-doctor`, `operator-state`. `doctor`/`operator-state` write discovery/lifecycle reports into the target `.hldspec/` control state on first touch (git/SpecKit read-only, not file-write read-only). |
...
| SpecKit Build Loop Supervision | `HLDspec build-status target: <path>` | current/read-only | `status` / `operator-state`. Both write discovery/lifecycle/branch-gate reports into the target `.hldspec/` control state on first touch (git/SpecKit read-only, not file-write read-only). |
| SpecKit Build Loop Supervision | `HLDspec reassess target: <path>` | current/read-only | `review`, `status`, sync/reassessment artifacts where present. `status` writes discovery/lifecycle/branch-gate reports into the target `.hldspec/` control state on first touch (git/SpecKit read-only, not file-write read-only). |
