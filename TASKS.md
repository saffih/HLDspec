...
- **Durable SpecKit invocation audit log** — contract ratified 2026-07-11: `docs/SPECKIT_INVOCATION_AUDIT_LOG_CONTRACT.md` (see `docs/HLDSPEC_DEVELOPMENT_BACKLOG.md` P1-019). Roadmap is Slices A-E in the contract. Slice A (path/schema) and Slice B (durable append writer) done, both in `hldspec/speckit_invocation_audit.py`: canonical pointer-aware path helper, closed schema-version-1 STARTED/FINISHED validator, deterministic NDJSON line serialization, plus `append_invocation_record` — exclusive-lock append, full parent-directory `fsync` chain, corruption/lifecycle-pair detection. No `SpecKitInvoker`/drive-loop wiring, no reader, no diagnostics, no invocation path calls the writer, no audit record is produced automatically — Slices C-E remain not started. Point-in-time proof records (`docs/FIRST_LIVE_E2E_PROOF.md`, `docs/SPECKIT_INVOKER_TASKS_HAIKU_LIVE_PROOF.md`) remain a separate evidence category, unaffected by this durable log.
