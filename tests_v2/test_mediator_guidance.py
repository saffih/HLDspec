    ...
    def test_rendered_prompts_preserve_journey3_boundaries(self) -> None:
        ...
        for text in (start, direct):
            with self.subTest(text="direct-mode shared"):
                ...
                # Generation is real now: guidelines are required, not "when present",
                # and the prompts must not claim HLDspec fails to auto-generate them.
                self.assertIn("engineering_guidelines.md was read", text)
                self.assertNotIn("(when present)", text)
                self.assertNotIn("HLDspec does not yet auto-generate it", text)
                self.assertIn("target/.hldspec/source_package/engineering_guidelines.md", text)
                self.assertIn(
                ...
                self.assertIn("generated engineering_guidelines.md is missing", text)
        ...
        self.assertIn("engineering_guidelines.md was read", devin)
        self.assertNotIn("(when present)", devin)
        self.assertNotIn("HLDspec does not yet auto-generate it", devin)
        self.assertIn(
            "engineering_guidelines.md is generated by HLDspec for new source packages; "
            "if missing in an older/manual workspace, stop and reassess.",
            devin,
        )
        self.assertIn("generated engineering_guidelines.md is missing", devin)
        ...
        self.assertIn("engineering_guidelines.md was read", direct)
        self.assertNotIn("(when present)", direct)
        self.assertNotIn("HLDspec does not yet auto-generate it", direct)
        self.assertIn(
            "engineering_guidelines.md is generated by HLDspec for new source packages; "
            "if missing in an older/manual workspace, stop and reassess.",
            direct,
        )
        self.assertIn("generated engineering_guidelines.md is missing", direct)
        for phrase in (
