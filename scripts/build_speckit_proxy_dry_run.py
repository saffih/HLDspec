...
def build_dry_run(workspace: Path, phase: str = "specify") -> dict[str, Any]:
    ...
    return {
        ...
        "approval": {
            ...
            "prework_package": str(sync / "speckit_prework_package.json"),
        },
        "state": {
...
def render_md(data: dict[str, Any]) -> str:
    ...
    if isinstance(reasons, list) and reasons:
        ...
        lines.append("")
    feature = data.get("selected_feature", {}) if isinstance(data.get("selected_feature"), dict) else {}
    lines += [
        "## Selected feature",
        "",
        f"- feature: `{feature.get('feature_id', '')}` - {feature.get('feature_name', '')}",
        f"- short name: `{feature.get('short_name', '')}`",
        "",
        "## Would run",
        "",
    ]
