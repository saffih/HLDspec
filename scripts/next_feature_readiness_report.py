...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--emit-agents-md",
        action="store_true",
        help="Also write the target-side agent-guidance bootstrap (next_feature_AGENTS.md) so an agent can drive the feature step by step.",
    )
    ...
    if args.emit_agents_md:
        paths = nfa.write_next_feature_agents_md(target, report)
        print(f"\nAgent guide written: {paths['agents_md']}\n")
