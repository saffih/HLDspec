...
def render_prompt(plan: dict[str, Any]) -> str:
    return (
        ...
        "- Also emit one HLD-DESC canonical line per section: `<ID> is <scope> <role> at <risk> risk, touching <surfaces>; \"<evidence>\".` "
        "Every value before the quote must come from hldspec/hld_vocabulary.json (scope: in_scope|out_of_scope|non_goal). "
        "Mark deliberately-excluded sections out_of_scope so downstream never derives requirements/surfaces from them; the quote is human-only evidence.\n\n"
        "Open:\n"
        f"- {plan['source_hld']}\n"
