...
"${PYTHON_RUN[@]}" "$ROOT/scripts/enrich_spec_build_plan_with_answer_context.py" "$WORKSPACE/.specify/sync/spec_build_plan.json" "$WORKSPACE"
"${PYTHON_RUN[@]}" "$ROOT/scripts/build_speckit_prework_plan.py" "$WORKSPACE/.specify/sync/spec_build_plan.json" "$WORKSPACE"
"${PYTHON_RUN[@]}" "$ROOT/scripts/build_speckit_bundle_queue.py" "$WORKSPACE"
...
"${PYTHON_RUN[@]}" "$ROOT/scripts/build_hld_answer_dossier.py" "$WORKSPACE"
"${PYTHON_RUN[@]}" "$ROOT/scripts/build_speckit_prework_quality_review.py" "$WORKSPACE"
"${PYTHON_RUN[@]}" "$ROOT/scripts/build_hldspec_orchestration_state.py" "$WORKSPACE"
"${PYTHON_RUN[@]}" "$ROOT/scripts/build_speckit_bundle_prompts.py" "$WORKSPACE"
"${PYTHON_RUN[@]}" "$ROOT/scripts/write_agent_context_handoff.py" "$WORKSPACE"
"${PYTHON_RUN[@]}" "$ROOT/scripts/validate_speckit_bundles.py" "$WORKSPACE"
"${PYTHON_RUN[@]}" "$ROOT/scripts/build_hldspec_state.py" "$WORKSPACE" --source-hld "$HLD_SOURCE"
"${PYTHON_RUN[@]}" "$ROOT/scripts/build_speckit_prework_package.py" "$WORKSPACE"
...
echo "- $WORKSPACE/.specify/sync/spec_build_plan_context_enrichment.md"
...
echo "- $WORKSPACE/.specify/sync/speckit_input_manifest.md"
echo "- $WORKSPACE/.specify/sync/speckit_invocation_queue.md"
echo "- $WORKSPACE/.specify/sync/speckit_bundle_queue.md"
echo "- $WORKSPACE/.specify/sync/speckit_bundle_prompts/README.md"
echo "- $WORKSPACE/.hldspec-run/hldspec-gap-agent-context-handoff.md"
echo "- $WORKSPACE/.specify/sync/validation/speckit_bundle_validation.md"
...
echo "- $WORKSPACE/.specify/sync/speckit_prework_package.md"
echo "- $WORKSPACE/.specify/sync/speckit_answer_dossier.md"
echo "- $WORKSPACE/.specify/sync/hld_answer_dossier_quality_review.md"
echo "- $WORKSPACE/.specify/sync/interface_contract_map.md"
echo "- $WORKSPACE/.specify/sync/project_vocabulary.md"
echo "- $WORKSPACE/.specify/sync/hldspec_state.md"
...
echo "2. Review SpecKit prework package: speckit_prework_package.md."
echo "   Supporting artifacts: manifest, invocation queue, constitution update plan, dependency graph, quality review, proxy dossier, bundle prompts, and the agent-context handoff."
echo "3. If the plan is green, run the bundle prompts, then read .hldspec-run/hldspec-gap-agent-context-handoff.md before any target-root agent handoff."
echo "4. Discuss and approve/fix the plan before invoking SpecKit."
echo "5. Resolve flagged specs by editing the working HLD.md or HLD-SPECS mapping when needed."
echo "6. Re-run this script with the edited HLD."
echo
